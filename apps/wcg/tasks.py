...
def import_people():
    print(f"task import_people running...")
    try:
        db["person"].insert(**{
            "firstname": "Sam",
            "surname": "Theman",
            "email": "sam.theman@gmail.com",
            "username": "samtheman"}
        )
        db.commit()
    except:
...
if settings.USE_SCHEDULER:
    ...
    scheduler.register_task("import_people", import_people)
    ...
    if db(db.task_run).count() < 1:
        ...
        scheduler.enqueue_run("import_people", inputs={}, timeout=2, period=10)
